    ...
    while found:
        ...
        for node in nodes:
            ...
            if len(graph.edges(node)) <= 1:
                ...
                if graph.has_edge(neigh[0], neigh[1]):
                    continue
                graph.add_edge(neigh[0], neigh[1])
                graph.remove_node(node)
                found = True
        if len(graph.edges(terminals[0])) == 1:
            neigh = list(graph.neighbors(terminals[0]))
            graph.remove_node(terminals[0])
            terminals[0] = neigh[0]

        if len(graph.edges(terminals[1])) == 1:
            neigh = list(graph.neighbors(terminals[1]))
            graph.remove_node(terminals[1])
            terminals[1] = neigh[0]
