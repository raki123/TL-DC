...
typedef std::priority_queue<std::pair<Edge_length, Vertex>,
                            std::vector<std::pair<Edge_length, Vertex>>,
...
private:
  ...
  components(const std::set<Vertex> &forbidden);
  std::vector<Vertex> find_separator(size_t size, size_t min_component_size,
                                     bool terminals_in_same = true);
