...
Decomposer::Decomposer(const char *const decomposer,
                       const char *const path_decomposer, const char *params) {
...
std::pair<int, int> Decomposer::insertEdges(AnnotatedDecomposition &r,
                                            std::vector<vertex_t> &bag,
                                            std::vector<Edge> &edges,
                                            std::set<Edge> &used, size_t child,
                                            NodeType type, bool join) {
  int first = -1;
  int idx = -1;
  for (auto it = edges.begin();
       (join && it == edges.end() && first < 0) || it != edges.end();) {
    AnnotatedNode c;
...
/*int Decomposer::createNode(AnnotatedDecomposition& r, std::vector<vertex_t>&
bag, std::vector<Edge>::iterator& it, NodeType type)
{
        AnnotatedNode c;
        c.type = type;
        c.bag = bag;
...
void Decomposer::update_Join_bag(AnnotatedNode &c, std::vector<vertex_t> &b1,
                                 std::vector<vertex_t> &b2) {
  ...
  for (auto it = c.bag.begin(); it != c.bag.end();) {
    if (c1.count(*it) == 0 &&
        c2.count(*it) == 0) // new one, don't need in join node
      it = c.bag.erase(it);
    else
      ++it;
  }
...
Decomposer::tree_decompose(/*const*/ Graph &graph, bool path, size_t *width,
                           size_t *nr_bags, size_t *max_join_child_bags,
                           size_t *max_join_bag, size_t *nr_joins) {
  ...
  while (stack.size() > 0) { // actual_td.count(cur) != 0) {
    ...
    // std::cerr << cur.second << " pred: " << cur.first << " join: " <<
    // joins[cur.second] << std::endl;
    ...
    if (td2r.count(cur.second) == 0)
      idx =
          insertEdges(r, bag, edges, used, (size_t)cur.first,
                      cur.first < 0 ? LEAF : PATH_LIKE, joins[cur.second] > 1);
    else {
    ...
    /*if (idx.first == -1)	//no edges, skip it then
    {
            ...
            if (succ.count(cur.second) > 0) {	//only non-empty tds
                    ...
                    stack.push_back(std::make_pair(cur.first,
    succ[cur.second][0])); } else { std::cerr << cur.second << " DIES OUT, pred:
    " << cur.first  << std::endl;
            }
    ...
    else*/
    {
      if (idx.first != -1) {
        ...
        // std::cerr << "no insert " << cur.second << "," << idx.first <<
        // std::endl;
      ...
      if (succ.count(cur.second) > 0) {
        ...
        auto par = idx.first == -1 && td2r.count(cur.second) > 0
                       ? cur.second
                       : succ[cur.second][0];
        if (td2r.count(par) >
            0) // parent already exists, can only be a join node, right?
        {
          ...
          { // add intermediate join node
            ...
            if (r[ridx].type != JOIN)             // create edge-empty JOIN node
            {
              ...
              /*std::set<vertex_t> c1 = std::set<vertex_t>(cn.bag.begin(),
              cn.bag.end()); std::set<vertex_t> c2 =
              std::set<vertex_t>(bag.begin(), bag.end());*/

              // parent bag, compute a better one; more suited for join
              c.bag = r[ridx].bag; // bag;
              update_Join_bag(c, cn.bag, bag);
              /*for (auto it = c.bag.begin(); it != c.bag.end(); ) {
                      if (c1.count(*it) == 0 && c2.count(*it) == 0)	//new one,
              don't need in join node it = c.bag.erase(it); else
                              ++it;
              }*/
            ...
            if (max_join_child_bags != nullptr)
              *max_join_child_bags =
                  std::max(*max_join_child_bags,
                           r[join->children.first].bag.size() +
        ...
        // std::cerr << cur.second << " DIES OUT, pred: " << cur.first  <<
        // std::endl;
      }
    }
    ...
    /*if (std::get<2>(td).count(cur) == 0)	//no successor
            ...
            //FIXME: extend to TDs (first element / one successor sufficient for
    PDs) cur = std::get<2>(td)[cur][0];*/
  }
  // stats(r);
  return std::move(r);
}
...
Decomposer::path_decompose(/*const*/ Graph &graph) {
...
void AnnotatedNode::stats() const {
  std::cerr << "TYPE " << type << ", parent: " << parent << ", edge: ("
            << edge.first << "," << edge.second << ") children: ("
            << children.first << "," << children.second << ") bag {";
...
Td_t Decomposer::decompose(/*const*/ Graph &graph, bool path) {
  ...
  for (Vertex v = 0; v < graph.adjacency_.size(); v++) {
    ...
    // for (auto i=graph.neighbors(v).begin(); i!= graph.neighbors(v).end();
    // ++i) 	std::cerr << i << std::endl;
  }
  nr_edges /= 2;

  int in, out;
  int root = 0;
  if (popen2(path ? this->path_decomposer : this->decomposer,
             NULL /*this->params*/, &in, &out) > 0) {
    std::stringstream s;
    ...
    int i = 0;
    for (Vertex v = 0; v < graph.adjacency_.size(); v++) {
      // for (auto it = graph.neighbors_.begin(); it != graph.neighbors_.end();
      // ++it)
      //{
      // std::set<Vertex>& s = *it;
    ...
    if (fscanf(fout, "s td %d %d %d\n", &bags, &width, &verts) == 3) {
      ...
      for (; b <= bags && fgets(buf, BUF_SIZE - 1, fout) != NULL; ++b) {
        if (buf[0] != 'c') {
          ...
          for (auto jt = bag.begin(); jt != bag.end(); ++jt) {
            std::set<Vertex> ngbs = std::move(graph.neighbors(*jt));
            ...
            // set_intersection(bag.begin(), bag.end(), ngbs.begin(),
            // ngbs.end(), std::inserter(intersect, intersect.begin()));
            /*if ((*jt) == 18) {
                    std::cerr << *jt << "," << bag.size() << "," << ngbs.size()
            << "," << intersect.size() << std::endl; for (auto it =
            ngbs.begin(); it != ngbs.end(); ++it) std::cerr << "n " << *it <<
            std::endl; for (auto it = bag.begin(); it != bag.end(); ++it)
            ...
            for (auto it = intersect.begin(); it != intersect.end(); ++it) {
              ...
              if (*jt < *it) {
                ...
                if (edges.find(e) == edges.end()) {
                  edges.insert(e);
                  td.push_back(
                      e); //, std::vector<vertex_t>> (std::move(e), bag));
                }
              }
            }
          }
          bgs.insert({bid, std::pair<std::vector<Edge>, std::vector<vertex_t>>(
                               std::move(td), std::move(bag))});
        } else {
          if (root == 0)
            sscanf(buf, "c r %d\n", &root);
          b--;
      ...
      /*for (Vertex v = 0; v < graph.adjacency_.size(); v++)
      {
              ...
              for (auto jt = ngbs.begin(); jt != ngbs.end(); ++jt)
              {
                      if (v < *jt && edges.find(std::make_pair(v, (*jt))) ==
      edges.end())
                      {
                              std::cerr << "MISSING EDGE " << v << "," << *jt <<
      std::endl;
                      }
              }
      }*/

      // std::cerr << "edges " << nr_edges << "; found edges " << edges.size()
      // << std::endl;
      ...
      for (; b > 0 && fgets(buf, BUF_SIZE - 1, fout) != NULL &&
             sscanf(buf, "%d %d\n", &b1, &b2);
           --b)
  ...
  /*return std::pair<int, std::pair<std::map<int, std::vector<int>>,
     std::map<int, std::pair<std::vector<Edge>, std::vector<vertex_t>>>>>(root,
                  std::pair<std::map<int, std::vector<int>>, std::map<int,
     std::pair<std::vector<Edge>, std::vector<vertex_t>>>>(succ,
     std::move(bgs)));	*/
  return std::make_tuple(root, std::move(leaves), std::move(succ),
                         std::move(bgs));
}
