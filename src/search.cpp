...
Search::Search(Graph &input)
    : enable_dag_(true), max_length_(input.max_length_),
      terminals_(input.terminals_), neighbors_(input.neighbors_.size()),
      adjacency_(input.adjacency_.size(),
                 std::vector<std::vector<std::pair<Edge_length, Edge_weight>>>(
                     input.adjacency_.size())),
      invalid_(std::numeric_limits<Edge_length>::max() - max_length_ - 1),
      distance_to_goal_(adjacency_.size(), invalid_),
      distance_(adjacency_.size(),
                std::vector<Edge_length>(adjacency_.size(), invalid_)),
      exclude_(adjacency_.size()), visited_(adjacency_.size(), false),
  ...
  for (Vertex v = 0; v < adjacency_.size(); v++) {
    // fill neighbors
    neighbors_[v] = std::vector<Vertex>(input.neighbors_[v].begin(),
                                        input.neighbors_[v].end());
    for (Vertex w : neighbors_[v]) {
      adjacency_[v][w] = std::vector<std::pair<Edge_length, Edge_weight>>(
          input.adjacency_[v][w].begin(), input.adjacency_[v][w].end());
    }
...
std::vector<Edge_weight> Search::search(Vertex start, Edge_length budget) {
  ...
  // simple means:
  //      * there is no edge (can only happen when we have exclusion
  //      constraints)
  ...
  if (poss_non_dag.size() == 0 && enable_dag_) {
    ...
    for (auto v : poss_dag) {
      ...
      std::vector<Edge_weight> tmp =
          dag_search(v, budget - adjacency_[start][v].begin()->first);
  ...
  if (!ap_components_.empty() && enable_dag_) {
    ...
    for (Vertex v : neighbors(start)) {
      if (!visited_[v]) {
        prev_distance_to_goal[start] =
            std::min(prev_distance_to_goal[start],
                     Edge_length(prev_distance_to_goal[v] +
                                 adjacency_[start][v].begin()->first));
      }
    }
    ...
    for (int i = cur_ap_components.size() - 1; i >= 0; i--) {
      ...
      assert(budget >=
             used_budget + prev_distance_to_goal[cur_ap_start_goal[i].second]);
      auto partial_res =
          search(cur_ap_start_goal[i].first,
                 budget - used_budget -
                     prev_distance_to_goal[cur_ap_start_goal[i].second]);
      // incorporate the partial result
      std::vector<Edge_weight> new_result(
          budget + 1 - prev_distance_to_goal[cur_ap_start_goal[i].second], 0);
      for (Edge_length prev_length = 0; prev_length < result.size();
           prev_length++) {
        for (Edge_length cur_length = 0;
             cur_length + prev_length < new_result.size(); cur_length++) {
          new_result[prev_length + cur_length] +=
              result[prev_length] * partial_res[cur_length];
        }
      }
      result = new_result;
      used_budget += (prev_distance_to_goal[cur_ap_start_goal[i].first] -
                      prev_distance_to_goal[cur_ap_start_goal[i].second]);
    }
  ...
  if (cached_result != cache_[start][terminals_[1]].end()) {
    if (cached_result->second.first >= budget) {
      ...
      std::vector<Edge_weight> ret(cached_result->second.second.begin(),
                                   cached_result->second.second.begin() +
                                       budget + 1);
      return ret;
    }
  }
  neg_hits++;
  auto cached_position = cache_[start][terminals_[1]].insert(std::make_pair(
      ap_visited_, std::make_pair(budget, std::vector<Edge_weight>())));
  std::vector<Edge_weight> ret(budget + 1, 0);
  for (auto v : poss_dag) {
    std::vector<Edge_weight> tmp;
    if (enable_dag_) {
  ...
  for (auto v : poss_non_dag) {
    std::vector<Edge_weight> tmp =
        search(v, budget - adjacency_[start][v].begin()->first);
    for (size_t i = 0; i < tmp.size(); i++) {
      for (auto &[length, weight] : adjacency_[start][v]) {
        if (length + i > budget) {
          break;
...
std::vector<Edge_weight> Search::dag_search(Vertex start, Edge_length budget) {
  ...
  while (!biggest_first_queue.empty()) {
    ...
    for (auto &w : neighbors(cur_vertex)) {
      ...
      } else if (distance_to_goal_[w] + edge_cost ==
                 distance_to_goal_[cur_vertex]) {
        if (!in_queue[w]) {
...
bool Search::ap_util(Vertex u, std::vector<char> &visited,
                     std::vector<Vertex> &disc, std::vector<Vertex> &low,
                     int &time, int parent, Vertex start) {
...
void Search::pruning_dijkstra(Vertex start, Vertex prune,
  ...
  while (!queue.empty()) {
    ...
    for (auto &w : neighbors(cur_vertex)) {
      ...
      if (cur_cost + min_cost < distance[w] &&
          cur_cost + min_cost + distance_[prune][w] <= budget) {
        distance[w] = min_cost + cur_cost;
        queue.push_back(w);
      }
    }
