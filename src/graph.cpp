...
Graph::Graph(std::istream &input) {
  ...
  while (!input.eof()) {
    ...
    case 'p': {
      ...
      assert(str == "edge");
      adjacency_ = std::vector<std::vector<std::map<Edge_length, Edge_weight>>>(
          nr_vertices, std::vector<std::map<Edge_length, Edge_weight>>(
                           nr_vertices, std::map<Edge_length, Edge_weight>()));
      neighbors_ =
          std::vector<std::set<Vertex>>(nr_vertices, std::set<Vertex>());
      exclusion_classes_ =
          std::vector<std::set<Vertex>>(nr_vertices, std::set<Vertex>());
    ...
    case 't':
      ...
      break;
    default:
      std::cerr << "Invalid character " << dec << " at beginning of line."
                << std::endl;
      break;
    }
    input >> dec;
...
void Graph::print_stats() {
  ...
  if (position_determined_removed)
    std::cerr << "Removed position determined: " << position_determined_removed
              << std::endl;
  if (twin_edges_removed)
  ...
  if (unusable_edge_removed)
    std::cerr << "Removed unusable edge: " << unusable_edge_removed
              << std::endl;
  if (two_sep_removed)
    std::cerr << "Removed due to 2-separation: " << two_sep_removed
              << std::endl;
  if (three_sep_removed)
    std::cerr << "Removed due to 3-separation: " << three_sep_removed
              << std::endl;
  if (max_length_decrease)
    std::cerr << "Max length decreased by: " << max_length_decrease
              << std::endl;
  std::cerr << "#vertices " << adjacency_.size() << " #edges " << nr_edges;
  if (!all_pair_) {
    std::cerr << " max. length " << static_cast<size_t>(max_length_)
              << " min. length "
              << static_cast<size_t>(distance_to_goal[terminals_[0]])
              << std::endl;
    std::cerr << "terminals: " << terminals_[0] << "," << terminals_[1]
              << std::endl;
  } else {
    std::cerr << std::endl;
  }
...
void Graph::normalize(bool reorder) {
  ...
  auto new_adjacency =
      std::vector<std::vector<std::map<Edge_length, Edge_weight>>>(
          cur_name, std::vector<std::map<Edge_length, Edge_weight>>(
                        cur_name, std::map<Edge_length, Edge_weight>()));
  auto new_neighbors =
      std::vector<std::set<Vertex>>(cur_name, std::set<Vertex>());
  for (Vertex v = 0; v < adjacency_.size(); v++) {
    if (neighbors(v).empty()) {
      continue;
    }
...
void Graph::add_exclude(Vertex v, Vertex w) {
  ...
  exclusion_classes_[exclude_[v]].insert(
      exclusion_classes_[exclude_[w]].begin(),
      exclusion_classes_[exclude_[w]].end());
...
Graph::Graph(Vertex n)
    : max_length_(-1), neighbors_(n, std::set<Vertex>()),
      adjacency_(n, std::vector<std::map<Edge_length, Edge_weight>>(
                        n, std::map<Edge_length, Edge_weight>())),
      exclusion_classes_(n), exclude_(n) {
  ...
  for (Vertex v : restrict_to) {
    ...
    if (new_exclude_name[exclude_[v]] == unnamed) {
      ...
      for (auto excluded : exclusion_classes_[exclude_[v]]) {
        ...
        ret.exclusion_classes_[new_exclude_name[exclude_[v]]].insert(
            new_name[excluded]);
      }
    }
...
void Graph::dijkstra(Vertex start, std::vector<Edge_length> &distance,
                     const std::set<Vertex> &forbidden) {
  DijkstraQueue queue;
...
Graph::components(const std::set<Vertex> &forbidden) {
...
std::vector<Vertex> Graph::find_separator(size_t size,
  ...
  if (handle.get().is_satisfiable()) {
    for (auto symbol : handle.model().symbols()) {
      auto substr =
          symbol.to_string().substr(4, symbol.to_string().length() - 4 - 1);
...
Vertex Graph::preprocess_forwarder() {
  ...
  for (size_t v = 0; v < adjacency_.size(); v++) {
    ...
    if (cur_neighbors.size() == 2) {
      found++;
      // if the neighbors are w1 and w2, then we can use any combination of
      // weighted edges (w1,v),(v,w2) as a single edge (w1,w2)
      Vertex w1 = *cur_neighbors.begin(), w2 = *(++cur_neighbors.begin());
      for (auto w1_weight : adjacency_[v][w1]) {
        for (auto w2_weight : adjacency_[v][w2]) {
          add_edge(Edge(w1, w2), Weight(w1_weight.first + w2_weight.first,
                                        w1_weight.second * w2_weight.second));
        }
      }
      remove_vertex(v);
    }
...
Vertex Graph::preprocess_twins() {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    assert(std::find(neighs.begin(), neighs.end(), terminals_[1 - i]) ==
           neighs.end());
    // first make sure that none of the neighbors are fixed because then we
    // cannot touch them here
    ...
    // gather equivalent vertices (i.e. twins) and merge them into one vertex
    // each
    ...
    for (size_t j = 0; j < neighs.size(); j++) {
      for (size_t k = j + 1; k < neighs.size(); k++) {
        Vertex w1 = neighs[j];
        Vertex w2 = neighs[k];
        // compare the neighborhoods of w1 and w2 to see if they are equal (up
        // to w1/w2)
        bool problem = false;
        ...
        for (auto &wp : neighbors(v)) {
          ...
          for (auto &weight : adjacency_[v][w]) {
            for (auto &weightp : adjacency_[v][wp]) {
              to_add.push_back(std::make_tuple(Edge(w, wp),
                                               weight.first + weightp.first,
                                               weight.second * weightp.second));
            }
          }
        }
      }
...
Vertex Graph::preprocess_two_separator() {
  ...
  // otherwise we can separate if the start or the goal have less than two
  // neighbors
  ...
  for (auto &comp : comps) {
    ...
    bool found_goal =
        std::find(comp.begin(), comp.end(), terminals_[1]) != comp.end();
    ...
    if (found_goal || found_start) {
      ...
      comp_graph.normalize();
      search =
          ParallelSearch(comp_graph.to_canon_nauty(true), c_one_length, 12);
      res = search.search();
      ...
      comp_graph.normalize();
      search =
          ParallelSearch(comp_graph.to_canon_nauty(true), c_two_length, 12);
      res = search.search();
      res_extra = comp_graph.extra_paths();
      res.resize(max_length_ + 1);
      res_extra.resize(max_length_ + 1);
      ...
      // remove all vertices in the component (apart from the separator, the
      // terminal, and two vertices that we will reuse)
      ...
      for (Edge_length length = 0; length <= max_length_; length++) {
        if (c_one_n[length] > 0) {
          add_edge(Edge(terminal, separator[0]),
                   Weight(length, c_one_n[length]));
        }
        if (c_two_n[length] > 0) {
          add_edge(Edge(terminal, separator[1]),
                   Weight(length, c_two_n[length]));
        }
        if (c_one_y[length] - c_one_n[length] > 0) {
          add_edge(Edge(comp[0], separator[0]),
                   Weight(length - 1, c_one_y[length] - c_one_n[length]));
        }
        if (c_two_y[length] - c_two_n[length] > 0) {
          add_edge(Edge(comp[1], separator[1]),
                   Weight(length - 1, c_two_y[length] - c_two_n[length]));
        }
      }
    ...
    if (max_length_ >
        distance_from_start[separator[0]] + distance_to_goal[separator[1]]) {
      d1 = max_length_ - distance_from_start[separator[0]] -
           distance_to_goal[separator[1]];
    }
    if (max_length_ >
        distance_from_start[separator[1]] + distance_to_goal[separator[0]]) {
      d2 = max_length_ - distance_from_start[separator[1]] -
           distance_to_goal[separator[0]];
    }
    ...
    ParallelSearch search(comp_graph.to_canon_nauty(true),
                          comp_graph.max_length_, 12);
    auto res = search.search();
    // search.print_stats();
    auto res_extra = comp_graph.extra_paths();
...
Vertex Graph::preprocess_three_separator() {
  Vertex found = 0;
  // here we can only do something if neither start nor goal are in the
  // component furthermore, we at least need
  ...
  for (auto &comp : comps) {
    ...
    // exclude each other (e_2 in principle already excludes e_1,e_3,e_5 so
    // excluding e_0,e_4 is enough) e_4 and all other e_i's exclude each other
    // (e_4 in principle already excludes e_1,e_3,e_5 so excluding e_0,e_2 is
    // enough)
    ...
    for (size_t i = 0; i < 3; i++) {
      ...
      for (size_t j = 0; j < 2; j++) {
        ...
        Edge_length d1 = 0, d2 = 0;
        if (max_length_ > distance_from_start[separator[start_idx]] +
                              distance_to_goal[separator[goal_idx]]) {
          d1 = max_length_ - distance_from_start[separator[start_idx]] -
               distance_to_goal[separator[goal_idx]];
        }
        if (max_length_ > distance_from_start[separator[goal_idx]] +
                              distance_to_goal[separator[start_idx]]) {
          d2 = max_length_ - distance_from_start[separator[goal_idx]] -
               distance_to_goal[separator[start_idx]];
        }
        comp_graph.max_length_ = std::max(d1, d2);
        comp_graph.terminals_ = {0, 1};
    ...
    // remove all vertices in the component (apart from the separator, the
    // terminal, and two vertices that we will reuse) we keep the first six to
    // reuse
    ...
    for (Edge_length length = 2; length <= max_length_; length++) {
      for (size_t i = 0; i < 3; i++) {
        if (counts[(i + 1) % 3][1][length] > 0) {
          add_edge(Edge(separator[i], comp[(2 * (i + 2)) % 6]),
                   Weight(length - 1, counts[(i + 1) % 3][1][length]));
        }
        if (counts[(i + 1) % 3][0][length] - counts[(i + 1) % 3][1][length] >
            0) {
          add_edge(Edge(separator[i], comp[(2 * (i + 2) + 1) % 6]),
                   Weight(length - 1, counts[(i + 1) % 3][0][length] -
                                          counts[(i + 1) % 3][1][length]));
        }
      }
    }
...
Vertex Graph::limit_max_length() {
  ...
  for (Vertex v = 0; v < adjacency_.size(); v++) {
    ...
    for (auto &[w, length] : edges) {
      prog_str << "reach(" << w << ",L + L') :- reach(" << v << ", L), edge("
               << v << "," << w << ",L'), L <= "
               << static_cast<size_t>(max_length_ - distance_to_goal[w] -
                                      length);
      prog_str << ", L>= " << static_cast<size_t>(distance_from_start[v])
               << ".\n";
      prog_str << "edge(" << v << "," << w << "," << static_cast<size_t>(length)
               << ") :- reach(" << v << ", L), L <= "
               << static_cast<size_t>(max_length_ - distance_to_goal[w] -
                                      length);
      prog_str << ", L>= " << static_cast<size_t>(distance_from_start[v]);
      for (auto &[wp, lengthp] : edges) {
        if (w != wp || length != lengthp) {
          prog_str << ", not edge(" << v << "," << wp << ","
                   << static_cast<size_t>(lengthp) << ")";
        }
      }
      prog_str << ".\n";
    }
  ...
  while (impossible) {
    impossible = false;
    auto atom = Clingo::Function(
        "reach", {Clingo::Number(terminals_[1]), Clingo::Number(best_cost)});
