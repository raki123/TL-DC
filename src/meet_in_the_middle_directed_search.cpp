...
MITMDirectedSearch<Count_structure, count_t>::MITMDirectedSearch(
    Digraph const &input, size_t nthreads)
    : nr_vertices_(input.nr_vertices()), nthreads_(nthreads),
      max_length_(
          std::min(input.max_length(), Edge_length(input.nr_vertices()))),
      terminals_(input.terminals()), graph_(input), in_neighbors_(nr_vertices_),
      out_neighbors_(nr_vertices_),
  ...
  input.dijkstra(terminals_[0], distance_from_start_, true);
  input.dijkstra(terminals_[1], distance_to_goal_, false);
  omp_set_num_threads(nthreads_);
}

template <template <typename> typename Count_structure, typename count_t>
mpz_class MITMDirectedSearch<Count_structure, count_t>::search() {
...
MITMDirectedSearch<Count_structure, count_t>::percent_done_after(
  ...
  auto compute_for = [&](auto mid) {
    ...
    graph_.dijkstra(mid, distance_from_mid, true);
    ...
    while (!stack.empty()) {
      ...
      if (neigh_idx < in_neighbors(u).size()) {
        ++steps_taken;
        ...
        auto v = in_neighbors(u)[neigh_idx];
        if (visited[v]) {
          continue;
        }
        if (length + 1 + distance_from_start_[v] > ms_max_len) {
...
template <template <typename> typename Count_structure, typename count_t>
void MITMDirectedSearch<Count_structure, count_t>::insert_all_subsets(
...
template <template <typename> typename Count_structure, typename count_t>
void MITMDirectedSearch<Count_structure, count_t>::enumerate_ms_paths(
...
template <template <typename> typename Count_structure, typename count_t>
void MITMDirectedSearch<Count_structure, count_t>::inclusion_exclusion(
    std::vector<Vertex> const &path, Vertex mid, Edge_length length) {
  auto &count = cache_[mid];
  auto sorted_path = path;
...
template <template <typename> typename Count_structure, typename count_t>
void MITMDirectedSearch<Count_structure, count_t>::enumerate_mt_paths(
    Vertex mid, Edge_length ms_max_len, Edge_length mt_max_len) {
  std::vector<Edge_length> distance_to_mid(nr_vertices_, invalid_);
  graph_.dijkstra(mid, distance_to_mid, false);
  ...
  while (!stack.empty()) {
    ...
    if (neigh_idx < out_neighbors(u).size()) {
      stack.back().neigh_idx++;
      auto v = out_neighbors(u)[neigh_idx];
      if (visited[v]) {
        continue;
      }
...
template <template <typename> typename Count_structure, typename count_t>
void MITMDirectedSearch<Count_structure, count_t>::print_stats() const {}
