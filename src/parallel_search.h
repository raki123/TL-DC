...
public:
    size_t operator()(PCacheKey const& key) const {
        return hasher__((const char *)key.v, sizeof(edge_t)*(key.nv + key.nde) + sizeof(degree_t)*key.nv);
    }  
};
struct sg_equal {
public:
    bool operator()(PCacheKey const& lhs, PCacheKey const& rhs) const {
        return lhs.nv == rhs.nv && lhs.nde == rhs.nde && std::memcmp(lhs.v, rhs.v, sizeof(edge_t)*(lhs.nv + lhs.nde) + sizeof(degree_t)*lhs.nv) == 0;
    }
};
