...
void ParallelNautySearch::add_to_initial(std::vector<sparsegraph> &to_add) {
  for (auto sg : to_add) {
...
std::vector<Edge_weight> ParallelNautySearch::search() {
  // cached vectors are {offset, results}
  Vertex nr_vertices = initial_.nv;
  for (Vertex remaining_size = nr_vertices + 1; remaining_size-- > 0;) {
    LOG << "\r" << nr_vertices + 1 - remaining_size << " / " << nr_vertices;
    LOG << ", with " << cache_[remaining_size].size() << " entries.";
...
Edge_weight ParallelNautySearch::dag_search(
    sparsegraph const &sg, Vertex start,
...
void ParallelNautySearch::prune_articulation(
    sparsegraph const &sg, Vertex start, std::vector<Edge_length> &distance) {
...
bool ParallelNautySearch::ap_util(sparsegraph const &sg, Vertex u,
                                  std::vector<char> &visited,
                                  std::vector<Vertex> &disc,
                                  std::vector<Vertex> &low, int &time,
                                  int parent, Vertex start,
...
void ParallelNautySearch::prune_util(sparsegraph const &sg, Vertex u,
                                     std::vector<Edge_length> &distance) {
  for (int i = 0; i < sg.d[u]; i++) {
    Vertex v = sg.e[sg.v[u] + i];
...
void ParallelNautySearch::pruning_dijkstra(sparsegraph const &sg,
                                           std::vector<Edge_length> &distance,
                                           Edge_length budget) {
...
void ParallelNautySearch::reverse_pruning_dijkstra(
...
void ParallelNautySearch::print_stats() {
