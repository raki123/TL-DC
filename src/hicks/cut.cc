...
int branchdecomp(int nodes, int edges, int *end1, int *end2, int &width,
  ...
  // std::cout<<"The branchwidth of the graph "<<argv[1]<<" is <=
  // "<<b.width<<"\n";
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 0; i < edges; i++) {
    ...
    auto first = min(end1[i], end2[i]);
    auto second = max(end1[i], end2[i]);
    end1[i] = first;
    end2[i] = second;
    end1[i]--;
    end2[i]--;
  ...
  */
  ...
  std::cout << "s bd " << g.edges << " " << b.nodes << " " << b.width << " "
            << b.edges << "\n";
  for (int i = 0; i < g.edges; i++) {
      ...
      if (g.nlist[i].art >= 1) {
        for (int j = 1; j < g.nlist[i].degree; j++) {
          ...
          if (g.elist[g.nlist[i].adj[j]].tree !=
            ...
            // std::cerr<<i<<" art node\n";
            // std::cerr<<g.elist[g.nlist[i].adj[j]].index<<" with bcomp
            // "<<g.elist[g.nlist[i].adj[j]].tree<<"\n";
            // std::cerr<<g.elist[g.nlist[i].adj[0]].index<<" with bcomp
            // "<<g.elist[g.nlist[i].adj[0]].tree<<"\n";
...
void bdecomp(graph &bigg, graph &g, bgraph &b) {
  int cuts = -1, *cutx = nullptr, *side1 = nullptr, *side2 = nullptr, es1 = 0,
      es2 = 0;
  // std::cerr<<"hello\n";
...
void create(int nodes, int edges, int *end1, int *end2,
            graph &g) { // Note: end1 has to min of the two
  g.width = 0;
  g.nodes = nodes;
  g.edges = edges;
...
void onecut(graph &g, int &cuts, int *&cutx, int &es1, int *&side1, int &es2,
  ...
  /*for(int i=0; i<g.edges; i++){
    //std::cerr<<g.nlist[g.elist[i].end1].index<<"
  "<<g.nlist[g.elist[i].end2].index;
    //std::cerr<<" g's edges\n";
  }
  for(int i=0; i<g.nodes; i++){
    g.nlist[i].number=0;
      ...
      /*
        ...
        for(int i=0; i<g.edges; i++){
          if(g.nlist[g.elist[i].end1].color==1
            ...
            for(int j=0; j<g.nlist[v].degree; j++){
              int other=g.elist[g.nlist[v].adj[j]].end1
        +g.elist[g.nlist[v].adj[j]].end2 - v; if(g.nlist[other].cut==1){
                count++;
              }
              else{
                g.nlist[other].color=0;
            ...
            for(int j=0; j<g.nlist[w].degree; j++){
              int other=g.elist[g.nlist[w].adj[j]].end1
        +g.elist[g.nlist[w].adj[j]].end2 - w; if(g.nlist[other].cut==1){ c++;
              }
              else{
                g.nlist[other].color=0;
                //break;
  ...
  for (int i = 0; i < g.nodes; i++) {
    for (int j = i + 1; j < g.nodes; j++) {
      if ((int)a[j][i] == diam) {
        ...
        if (ok > 0 && (cuts < 0 || (share > oshare && tcut < cuts) ||
...
void showmecut(graph &g, int cuts, int es1, int es2, int *cutx, int *side1,
  ...
  if (es1 < es2) {
    ...
    for (int i = 0; i < es1; i++) {
      ...
      // std::cerr << g.nlist[g.elist[side1[i]].end2].index << " the other
      // side\n";
    }
  } else {
    // std::cerr << es2 << " es2\n";
    for (int i = 0; i < es2; i++) {
      ...
      // std::cerr << g.nlist[g.elist[side2[i]].end2].index << " the other
      // side\n";
    }
  }
  // std::cerr << es1 << " es1\n";
...
int push(int v, int u, int e, int source, int sink, int cut, node *nlist,
         edge *elist) {
  int ok, j = -1;
  int w, k, t;
    ...
    if (g.nlist[w].number < 0) {
      ...
      if (g.nlist[w].lowpt >= g.nlist[v].number) {
        ...
        while (
          ...
          // std::cerr<<g.nlist[g.elist[*(head+1)].end1].index<<" and
          // "<<g.nlist[g.elist[*(head+1)].end2].index; std::cerr<<" are in comp
          // "<<comp<<" \n";
...
void leightonrao(graph &g) {
    ...
    while (weight + next >= (1.0 + flux * eps) * weight) {
      ...
      for (int i = 0; i < g.edges; i++) {
        ...
        if ((g.nlist[e1].color == 0 && g.nlist[e2].color == 1) ||
          ...
          if (ee < 0 ||
              (g.elist[i].lr - g.nlist[e1].number - g.nlist[e1].number) <
  ...
  int t = 0;
  for (int i = 0; i < g.edges; i++) {
    if (g.nlist[g.elist[i].end1].color < 0 &&
        g.nlist[g.elist[i].end2].color < 0) {
      // std::cerr << g.elist[i].end1 << " & " << g.elist[i].end2 << " color
      // cut\n";
      g.elist[i].tree = 1;
      t++;
    }
...
void findcut3(graph &g, int &cuts, int *&cutx, int &es1, int *&side1, int &es2,
  ...
  for (int i = 0; i < g.edges; i++) {
    ...
    // std::cerr<<g.nlist[g.elist[i].end1].index<<" the graph
    // "<<g.nlist[g.elist[i].end2].index<<"\n";
  }
  ...
  if (cuts == 0) {
    ...
    // std::cerr << es1 << " side1 and side2 " << es2 << " and edges " <<
    // g.edges << "\n";
    for (int i = 0; i < g.nodes; i++) {
      if (g.nlist[i].x != 0) {
    ...
    for (int i = 0; i < g.edges; i++) {
      // std::cerr<<g.nlist[g.elist[i].end1].index<<" and
      // "<<g.nlist[g.elist[i].end2].index<<" edges\n";
    }
    exit(-1);
  }
}
...
void dfs(graph &g, int comp, int u) {
  ...
  for (int i = 0; i < g.nlist[u].degree; i++) {
    int w =
        g.elist[g.nlist[u].adj[i]].end1 + g.elist[g.nlist[u].adj[i]].end2 - u;
    if (g.nlist[w].number == 0) {
      dfs(g, comp, w);
    }
  }
...
void center(graph &g) {
  ...
  while (ok) {
    ...
    if (v < 0) {
      ...
      // start=1;
      // end=start;
      // std::cerr<<start<<" start and end "<<end<<" for v
      // "<<g.nlist[v].index<<"\n"; std::cerr<<v<<" v\n"; for(int i=0;i<g.nodes;
      // i++){ std::cerr<<vert[i]<<" has "<<weight[i]<<" weight\n";
      // }
      ...
      for (int top = start; top >= end; top -= step) {
        ...
        if (okk >= 0) {
          int ecount = 0;
          for (int i = 0; i < g.edges; i++) {
            if (g.nlist[g.elist[i].end1].x > 0 &&
                g.nlist[g.elist[i].end2].x > 0) {
              ecount++;
            }
          }
          delete[] set;
          ...
          // std::cerr<<g.nlist[v].index<<" v and superv
          // "<<g.nlist[superv].index<<"\n"; std::cerr<<work<<" this";
          // std::cerr<<" should be less than this "<<owork<<" to be the
          // best\n"; if(big <=2*mini){
...
void flowgraph(graph &g, int num, int split, int *set, int &source, int &sink,
  ...
  for (int i = 0; i < g.edges; i++) {
    ...
    // std::cerr<<g.nlist[u].color<<" and "<<g.nlist[v].color<<" their
    // colors\n";
    // }
...
void eigengraph(graph &g) {
  ...
  for (int i = 0; i < g.edges; i++) {
    e[i] = i;
    // std::cerr<<y[i]<<" of "<<i<<" "<<g.nlist[g.elist[i].end1].index<<"
    // "<<g.nlist[g.elist[i].end2].index<<"\n";
  }
  ...
  */
  ...
  for (int i = 0; i < dum; i++) {
    ...
    // if(cn==dum) break;
    // std::cerr<<g.nlist[g.elist[e[i]].end1].index<<" source
    // "<<g.nlist[g.elist[e[i]].end2].index<<"\n";
  }
  for (int i = g.edges - 1; i >= g.edges - dum; i--) {
    ...
    // if(cn==dum) break;
    // std::cerr<<g.nlist[g.elist[e[i]].end1].index<<" sink
    // "<<g.nlist[g.elist[e[i]].end2].index<<"\n";
  }
...
void matmult1(graph &g, double *y, double *x) {
  ...
  for (int i = 0; i < g.nodes; i++) {
    ...
    for (int j = 0; j < g.nlist[i].degree; j++) {
      int u =
          g.elist[g.nlist[i].adj[j]].end1 + g.elist[g.nlist[i].adj[j]].end2 - i;
